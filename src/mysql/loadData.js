...
const loadReviews = (conn) => {
  ...
  return conn.query(
    'LOAD DATA INFILE ? INTO TABLE reviews FIELDS TERMINATED BY "," ENCLOSED BY \'"\' LINES TERMINATED BY "\n" IGNORE 1 ROWS (id,product_id,rating,@date,summary,body,@recommend,@reported,reviewer_name,reviewer_email,response,helpfulness) SET recommend = (@recommend = "true"), date = FROM_UNIXTIME(@date / 1000, "%Y-%m-%d"), reported = (@reported = "true")',
